...
-- 4. Create personality fit scores view (replace existing)
CREATE OR REPLACE VIEW v_personality_fit_scores AS
SELECT 
    dc.*,
    CASE 
        WHEN dc.personality_traits IS NOT NULL THEN
...
-- 5. Update confidence components view (if exists)
-- This will be handled in the next migration after testing