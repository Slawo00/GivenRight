-- Create a stored function that will deploy the views
CREATE OR REPLACE FUNCTION deploy_v3_views() 
RETURNS TEXT AS $$
DECLARE
    result TEXT := '';
BEGIN
    ...
    -- Update v_personality_fit_scores to use REAL scores (not hardcoded 0)
    EXECUTE 'CREATE OR REPLACE VIEW v_personality_fit_scores AS
    SELECT 
        dc.*,
        CASE 
            WHEN dc.personality_traits IS NOT NULL AND array_length(dc.personality_traits, 1) > 0 THEN
